...
def find_problem_products(soup, base_url, limit=15):
    """يدوّر على منتجات معينة فيها مشاكل واضحة ويرجّع روابطها المباشرة."""
    product_like = soup.select("[class*=product], [class*=Product], [id*=product], [class*=item-card]")
    seen_urls = set()
    problems = []
    for el in product_like:
        ...
        if href in seen_urls or href.rstrip("/") == base_url.rstrip("/"):
            continue
        seen_urls.add(href)

        img = el.find("img")
        name = link.get_text(strip=True) or (img.get("alt", "").strip() if img else "")
        el_text = el.get_text(" ", strip=True)
        has_price = bool(PRICE_PATTERN.search(el_text))

        issues = []
        if not name or len(name) < 3:
            issues.append("مفيش اسم واضح للمنتج")
        if not img or not img.get("src"):
            issues.append("مفيش صورة للمنتج")
        elif not (img.get("alt") or "").strip():
            issues.append("الصورة بدون alt text (بيأثر على SEO)")
        if not has_price:
            issues.append("مفيش سعر ظاهر جنب المنتج")
...
def build_audit_pdf(full_report):
    ...
    if problem_products:
        pdf_subtitle(pdf, f"منتجات محتاجة تعديل ({len(problem_products)})")
    ...
    if submitted:
        if not url.startswith("http"):
            ...
            if problem_products:
                st.markdown(f"""
                <div class="section-card">
                    <h3>🔗 منتجات محتاجة تعديل ({len(problem_products)})</h3>
                    <p style="color:#9aa4b2;">دي روابط مباشرة للمنتجات اللي فيها مشاكل واضحة، عشان توصلها بسهولة.</p>
                </div>
                """, unsafe_allow_html=True)
                for pp in problem_products:
                    with st.expander(f"⚠️ {pp['name']}"):
                        st.markdown(f"🔗 [افتح المنتج]({pp['url']})")
                        for issue in pp["issues"]:
                            st.write(f"❌ {issue}")
