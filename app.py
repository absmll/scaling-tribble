...
with st.sidebar:
    ...
    if history:
        with st.expander(f"🕓 سجل الفحوصات في هذه الجلسة ({len(history)})"):
            for h in history:
                st.markdown(
                    f"""<div style="font-size:0.8rem; color:#9aa4b2; margin-bottom:0.5rem;">
                    <b style="color:#f5e7a8;">{h['url']}</b><br>
                    SEO: {h['scores']['seo_score']} · UX: {h['scores']['ux_score']} ·
                    السرعة: {h['scores']['speed_score'] if h['scores']['speed_score'] is not None else '—'}
                    </div>""",
                    unsafe_allow_html=True,
                )
            st.caption("هذا السجل مؤقت لهذه الجلسة فقط، ولا يُحفظ بعد إغلاق الصفحة.")
