...
.stApp {
    background:
        radial-gradient(circle at top right, #1a1f2e 0%, #0d1117 55%, #0a0c10 100%);
}

/* Storefront shelf accent behind the hero, for a "shop window" feel */
.shelf-accent {
    height: 6px;
    border-radius: 6px;
...
.hero {
    background: linear-gradient(145deg, #1e2536 0%, #12151f 100%);
...
.section-card {
    background: linear-gradient(160deg, rgba(30,37,54,0.75), rgba(16,19,28,0.75));
...
.stButton > button, .stDownloadButton > button, .stLinkButton > a {
    ...
    box-shadow: 0 4px 0 rgba(0,0,0,0.25), 0 6px 14px rgba(0,0,0,0.3) !important;
    transition: transform 0.12s ease, box-shadow 0.12s ease !important;
}
.stButton > button:active, .stDownloadButton > button:active {
    transform: translateY(2px);
    box-shadow: 0 2px 0 rgba(0,0,0,0.25), 0 3px 8px rgba(0,0,0,0.3) !important;
}
.stButton > button[kind="primary"], .stDownloadButton > button[kind="primary"] {
    background: linear-gradient(135deg, #e4c158, #b8912c) !important;
    color: #10131c !important;
    box-shadow: 0 4px 0 #8a6a1f, 0 8px 18px rgba(212,175,55,0.35) !important;
}
...
.gauge {
    ...
    box-shadow: 0 10px 24px rgba(0,0,0,0.4), 0 2px 4px rgba(0,0,0,0.3);
}
.gauge-inner {
...
div[data-testid="stExpander"] {
    background: linear-gradient(160deg, rgba(30,37,54,0.7), rgba(16,19,28,0.7));
...
# ==================== حماية الوصول بكلمة مرور (اختياري) ====================
# تُفعَّل هذه الحماية فقط عند ضبط APP_PASSWORD ضمن Secrets في لوحة تحكم
# Streamlit Cloud. إن لم تُضبط، تبقى الأداة مفتوحة كما كانت (لا قفل افتراضي
# قد يمنعك من الدخول إن نسيت ضبطه قبل النشر).
...
if _APP_PASSWORD:
    ...
    if not st.session_state["authenticated"]:
        ...
        with _login_col:
            st.markdown("""
            <div class="shelf-accent"></div>
            <div class="hero" style="padding: 2.6rem 2rem;">
                <div style="font-size:2.6rem; margin-bottom:0.4rem;">🛍️</div>
                <h1 style="font-size:1.7rem;">أداة فحص المتاجر الإلكترونية</h1>
                <p>هذه الأداة محمية بكلمة مرور. يرجى إدخالها للمتابعة.</p>
            </div>
            """, unsafe_allow_html=True)
            _pw = st.text_input("كلمة المرور", type="password", label_visibility="collapsed",
...
def check_ssl_certificate(url):
    """يتحقق من تاريخ انتهاء شهادة SSL للنطاق، لتنبيه صاحب المتجر قبل انتهائها."""
    parsed = urlparse(url)
    ...
    try:
        ...
        return {"applicable": True, "expires_at": expire_dt.strftime("%Y-%m-%d"), "days_left": days_left}
    except Exception as e:
        return {"applicable": True, "error": str(e)}


def check_broken_links(soup, base_url, sample_size=12, timeout=6):
    """يفحص عيّنة من الروابط الداخلية للتأكد من عدم إرجاعها لصفحات غير موجودة."""
    ...
    for a in soup.find_all("a", href=True):
        ...
        if urlparse(full).netloc != base_netloc or full in seen:
            continue
        seen.add(full)
        links.append(full)
    ...
    for link in links:
        try:
            ...
            broken.append({"url": link, "status": "تعذّر الاتصال"})
    return {"checked": len(links), "broken": broken}


def check_trust_signals(html):
    """يفحص وجود عناصر تعزز ثقة المشتري وتقلل من هجر السلة."""
    return {
        "has_return_policy": bool(re.search(r"سياسة الاسترجاع|الاسترجاع والاستبدال|return\s?policy|refund\s?policy", html, re.I)),
        "has_shipping_info": bool(re.search(r"سياسة الشحن|الشحن والتوصيل|shipping\s?policy|shipping\s?info", html, re.I)),
        "has_privacy_policy": bool(re.search(r"سياسة الخصوصية|privacy\s?policy", html, re.I)),
        "has_terms": bool(re.search(r"الشروط والأحكام|terms\s?(and|&)\s?conditions", html, re.I)),
        "has_payment_badges": bool(re.search(r"mada|مدى|visa|فيزا|mastercard|ماستركارد|apple\s?pay|stc\s?pay|tabby|tamara", html, re.I)),
    }


def check_images_weight(soup, base_url, sample_size=8, timeout=6):
    """يقدّر متوسط حجم صور المنتجات عبر عيّنة، ويحدد الصور الثقيلة التي تبطئ الموقع."""
    imgs = [img for img in soup.find_all("img", src=True)][:sample_size]
    checked = 0
    total_bytes = 0
    oversized = []
    for img in imgs:
        ...
        try:
            r = requests.head(src, headers=HEADERS, timeout=timeout, allow_redirects=True)
            ...
            total_bytes += size
            if size > 200_000:
                oversized.append({"url": src, "size_kb": round(size / 1024, 1)})
        except (requests.RequestException, ValueError):
            continue
    avg_kb = round((total_bytes / checked) / 1024, 1) if checked else None
    return {"checked": checked, "avg_kb": avg_kb, "oversized": oversized}
...
def build_audit_pdf(full_report):
    ...
    ssl_info = full_report.get("ssl") or {}
    trust = full_report.get("trust")
    if ssl_info.get("applicable"):
        pdf_subtitle(pdf, "شهادة الأمان (SSL)")
        if "error" in ssl_info:
            pdf_line(pdf, f"تعذّر التحقق من الشهادة: {ssl_info['error']}", False)
        else:
            days = ssl_info["days_left"]
            pdf_line(pdf, f"تنتهي الشهادة في {ssl_info['expires_at']} (متبقٍ {days} يومًا)", days >= 30)

    if trust:
        pdf_subtitle(pdf, "مؤشرات الثقة")
        pdf_line(pdf, "سياسة الاسترجاع", trust["has_return_policy"])
        pdf_line(pdf, "سياسة الشحن", trust["has_shipping_info"])
        pdf_line(pdf, "سياسة الخصوصية", trust["has_privacy_policy"])
        pdf_line(pdf, "الشروط والأحكام", trust["has_terms"])
        pdf_line(pdf, "شعارات دفع موثوقة", trust["has_payment_badges"])

    broken_links = full_report.get("broken_links")
    if broken_links:
        pdf_subtitle(pdf, f"روابط معطلة ({len(broken_links['broken'])} من {broken_links['checked']} تم فحصها)")
        for b in broken_links["broken"]:
            pdf_line(pdf, f"{b['url']} — {b['status']}", False)

    images_weight = full_report.get("images_weight")
    if images_weight and images_weight["checked"]:
        pdf_subtitle(pdf, "حجم الصور")
        pdf_line(pdf, f"متوسط حجم الصورة (عيّنة من {images_weight['checked']}): {images_weight['avg_kb']} KB")
        for o in images_weight["oversized"]:
            pdf_line(pdf, f"{o['url']} — {o['size_kb']} KB", False)

    competitor = full_report.get("competitor")
    if competitor:
        pdf_subtitle(pdf, "مقارنة بمتجر المنافس")
        pdf_line(pdf, competitor["url"])
        cs = competitor["scores"]
        pdf_line(pdf, f"SEO المنافس: {cs['seo_score']}/100")
        pdf_line(pdf, f"UX المنافس: {cs['ux_score']}/100")
...
def render_audit_tab():
    ...
    with st.form("audit_form"):
        ...
        competitor_url = st.text_input("رابط متجر منافس للمقارنة (اختياري)", placeholder="https://competitor-store.com")
        api_key = st.text_input("مفتاح PageSpeed API (اختياري، لقياس درجة السرعة)", type="password")
        col_opt1, col_opt2 = st.columns(2)
        check_links = col_opt1.checkbox("فحص الروابط المعطلة", value=True)
        check_images = col_opt2.checkbox("فحص حجم الصور", value=True)
        ...
        if not url.startswith("http"):
            ...
            with st.spinner("جارٍ فحص المتجر..."):
                try:
                    ...
                    trust = check_trust_signals(html)
                    broken_links = check_broken_links(soup, url) if check_links else None
                    images_weight = check_images_weight(soup, url) if check_images else None
                except requests.RequestException as e:
                    st.error(f"تعذّر الوصول إلى الرابط: {e}")
                    st.stop()

            competitor = None
            if competitor_url.startswith("http"):
                with st.spinner("جارٍ فحص متجر المنافس..."):
                    try:
                        c_html, _, _ = fetch_html(competitor_url)
                        c_soup = BeautifulSoup(c_html, "html.parser")
                        c_seo = check_seo(c_soup, competitor_url)
                        c_ux = check_ux(c_soup, c_html, competitor_url)
                        c_pagespeed = check_pagespeed(competitor_url, api_key)
                        c_scores = score_report(c_seo, c_ux, c_pagespeed)
                        competitor = {"url": competitor_url, "scores": c_scores}
                    except requests.RequestException as e:
                        st.warning(f"تعذّر فحص متجر المنافس: {e}")
            ...
            if competitor:
                section_eyebrow("مقارنة بمتجر المنافس")
                st.markdown(f'<p style="color:#9aa4b2; margin-bottom:0.4rem;">متجرك (أعلى) مقابل {competitor["url"]} (أسفل)</p>', unsafe_allow_html=True)
                gauge_row([
                    ("SEO المنافس", competitor["scores"]["seo_score"]),
                    ("UX المنافس", competitor["scores"]["ux_score"]),
                    ("سرعة المنافس", competitor["scores"]["speed_score"]),
                ])
                deltas = []
                for key, label in [("seo_score", "SEO"), ("ux_score", "تجربة المستخدم")]:
                    diff = scores[key] - competitor["scores"][key]
                    if diff > 0:
                        deltas.append(f"متجرك أفضل في {label} بفارق {diff} نقطة")
                    elif diff < 0:
                        deltas.append(f"متجر المنافس أفضل في {label} بفارق {abs(diff)} نقطة")
                if deltas:
                    st.info(" • ".join(deltas))
            ...
            section_eyebrow("الموثوقية والثقة")

            if ssl_info.get("applicable"):
                if "error" in ssl_info:
                    ssl_line = f'<span style="color:#e74c3c;">✘ تعذّر التحقق من شهادة SSL: {ssl_info["error"]}</span>'
                else:
                    days = ssl_info["days_left"]
                    if days < 0:
                        ssl_line = f'<span style="color:#e74c3c;">✘ شهادة SSL منتهية بالفعل (انتهت في {ssl_info["expires_at"]})</span>'
                    elif days < 30:
                        ssl_line = f'<span style="color:#e74c3c;">⚠ شهادة SSL ستنتهي خلال {days} يومًا ({ssl_info["expires_at"]}) — يُنصح بالتجديد قريبًا</span>'
                    else:
                        ssl_line = f'<span style="color:#2ecc71;">✔ شهادة SSL سارية لمدة {days} يومًا (تنتهي في {ssl_info["expires_at"]})</span>'
            else:
                ssl_line = '<span style="color:#e74c3c;">✘ الموقع لا يستخدم HTTPS أصلًا</span>'

            st.markdown(f"""
            <div class="section-card">
                <h3>🔒 شهادة الأمان (SSL)</h3>
                <p>{ssl_line}</p>
            </div>

            <div class="section-card">
                <h3>🤝 مؤشرات الثقة</h3>
                {pill(trust['has_return_policy'], 'سياسة الاسترجاع')}
                {pill(trust['has_shipping_info'], 'سياسة الشحن')}
                {pill(trust['has_privacy_policy'], 'سياسة الخصوصية')}
                {pill(trust['has_terms'], 'الشروط والأحكام')}
                {pill(trust['has_payment_badges'], 'شعارات دفع موثوقة')}
            </div>
            """, unsafe_allow_html=True)

            if broken_links is not None:
                if broken_links["broken"]:
                    rows = "".join(
                        f'<p style="color:#9aa4b2; margin:0.3rem 0;">🔗 <a href="{b["url"]}" style="color:#e74c3c;">{b["url"]}</a> — {b["status"]}</p>'
                        for b in broken_links["broken"]
                    )
                    st.markdown(f"""
                    <div class="section-card" style="border-color: rgba(231,76,60,0.35);">
                        <h3 style="color:#e74c3c;">🔗 روابط معطلة ({len(broken_links['broken'])} من {broken_links['checked']} تم فحصها)</h3>
                        {rows}
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    ...
                    <div class="section-card">
                        <h3>🔗 روابط معطلة</h3>
                        <p style="color:#2ecc71;">لم يُرصد أي رابط معطل ضمن عيّنة من {broken_links['checked']} رابطًا داخليًا.</p>
                    </div>
                    """, unsafe_allow_html=True)

            if images_weight is not None and images_weight["checked"]:
                oversized_html = ""
                if images_weight["oversized"]:
                    oversized_html = "".join(
                        f'<p style="color:#9aa4b2; margin:0.3rem 0;">🖼️ {o["url"]} — {o["size_kb"]} KB</p>'
                        for o in images_weight["oversized"]
                    )
                st.markdown(f"""
                <div class="section-card">
                    <h3>🖼️ حجم الصور</h3>
                    <p style="color:#c9d1d9;">متوسط حجم الصورة (عيّنة من {images_weight['checked']}): <b>{images_weight['avg_kb']} KB</b></p>
                    {'<p style="color:#e74c3c;">صور تتجاوز 200 KB وتستحق ضغطًا:</p>' + oversized_html if images_weight['oversized'] else '<p style="color:#2ecc71;">لا توجد صور ثقيلة بشكل ملحوظ ضمن العيّنة المفحوصة.</p>'}
                </div>
                """, unsafe_allow_html=True)
            ...
            full_report = {
                ...
                "tracking": tracking, "ssl": ssl_info, "trust": trust,
                "broken_links": broken_links, "images_weight": images_weight,
            ...
            st.session_state["saved_reports"].insert(0, {
                "url": url, "scanned_at": full_report["scanned_at"], "scores": scores,
                "pdf_bytes": pdf_bytes, "json_data": json.dumps(full_report, ensure_ascii=False, indent=2),
            })
...
def render_reports_tab():
    hero("📁 التقارير المحفوظة", "آخر التقارير التي أصدرتها الأداة خلال هذه الجلسة، مع إمكانية إعادة تحميلها")

    reports = st.session_state.get("saved_reports", [])
    if not reports:
        st.info("لا توجد تقارير محفوظة بعد. اذهب إلى تبويب «فحص المتجر» وابدأ فحصًا لإنشاء أول تقرير.")
        return

    st.caption(f"عدد التقارير المحفوظة في هذه الجلسة: {len(reports)} (بحد أقصى 10، ولا تُحفظ بعد إغلاق الصفحة)")

    for i, r in enumerate(reports):
        score = r["scores"]
        speed = score["speed_score"] if score["speed_score"] is not None else "—"
        with st.expander(f"📄 {r['url']} — {r['scanned_at'][:16].replace('T', ' ')}"):
            st.markdown(f"""
            <p style="color:#c9d1d9;">
            SEO: <b style="color:#f5e7a8;">{score['seo_score']}/100</b> &nbsp;|&nbsp;
            تجربة المستخدم: <b style="color:#f5e7a8;">{score['ux_score']}/100</b> &nbsp;|&nbsp;
            السرعة: <b style="color:#f5e7a8;">{speed}</b>
            </p>
            """, unsafe_allow_html=True)
            col_pdf, col_json = st.columns(2)
            col_pdf.download_button(
                "📄 تحميل PDF", data=r["pdf_bytes"], file_name=f"report_{i+1}.pdf",
                mime="application/pdf", use_container_width=True, key=f"report_pdf_{i}",
            )
            col_json.download_button(
                "⬇️ تحميل JSON", data=r["json_data"], file_name=f"report_{i+1}.json",
                mime="application/json", use_container_width=True, key=f"report_json_{i}",
            )

    if st.button("🗑️ مسح كل التقارير المحفوظة"):
        st.session_state["saved_reports"] = []
        st.rerun()
...
def pdf_brand_header(pdf):
    provider_name = st.session_state.get("provider_name", "").strip()
    if provider_name:
        pdf.set_font("Arabic", "", 11)
        pdf.set_text_color(212, 175, 55)
        pdf.multi_cell(0, 7, ar(f"مقدَّم من: {provider_name}"), align="R")
        pdf.ln(1)
...
def render_salla_tab():
    ...
    if "salla_profiles" not in st.session_state:
        st.session_state["salla_profiles"] = {"عميل افتراضي": {}}
    if "active_salla_profile" not in st.session_state:
        st.session_state["active_salla_profile"] = "عميل افتراضي"

    profiles = st.session_state["salla_profiles"]

    st.markdown("##### 👥 ملف العميل")
    col_sel, col_new = st.columns([3, 2])
    with col_sel:
        selected = st.selectbox("اختر عميلًا محفوظًا", options=list(profiles.keys()),
                                 index=list(profiles.keys()).index(st.session_state["active_salla_profile"]))
        st.session_state["active_salla_profile"] = selected
    with col_new:
        new_name = st.text_input("إضافة عميل جديد باسم", placeholder="مثال: متجر الورد")
        if st.button("➕ إضافة", use_container_width=True) and new_name.strip():
            if new_name not in profiles:
                profiles[new_name] = {}
            st.session_state["active_salla_profile"] = new_name
            st.rerun()

    active = st.session_state["active_salla_profile"]
    profile = profiles[active]
    st.caption(f"يتم الآن التعامل مع بيانات: **{active}** — بيانات كل عميل منفصلة عن الآخر.")

    query_code = st.query_params.get("code")

    with st.expander("⚙️ إعدادات التطبيق لهذا العميل", expanded=not profile.get("token")):
        st.markdown(
        ...
        profile["client_id"] = st.text_input("Client ID", value=profile.get("client_id", ""), key=f"cid_{active}")
        profile["client_secret"] = st.text_input("Client Secret", type="password",
                                                   value=profile.get("client_secret", ""), key=f"cs_{active}")
        profile["redirect_uri"] = st.text_input("Redirect URI (رابط هذا الموقع)",
                                                  value=profile.get("redirect_uri", ""), key=f"ruri_{active}")

        if profile["client_id"] and profile["redirect_uri"]:
            auth_link = (
                f"{SALLA_AUTH_URL}?client_id={profile['client_id']}&response_type=code"
                f"&redirect_uri={profile['redirect_uri']}&scope=offline_access%20products.read_write"
            )
            st.link_button("🔗 ربط متجر سلة الآن", auth_link, use_container_width=True)

    if query_code and not profile.get("token"):
        cid, cs, ruri = profile.get("client_id"), profile.get("client_secret"), profile.get("redirect_uri")
        if cid and cs and ruri:
            try:
                token_data = salla_exchange_code(cid, cs, ruri, query_code)
                profile["token"] = token_data.get("access_token")
    ...
    if not profile.get("token"):
        st.info("لا يوجد متجر مرتبط بهذا العميل حاليًا. افتح الإعدادات أعلاه واضغط رابط الربط.")
        return

    st.success("المتجر مرتبط ✅")
    if st.button("إلغاء الربط لهذا العميل"):
        profile["token"] = None
        profile["products"] = []
        st.rerun()
    ...
    if st.button("🔄 جلب المنتجات", type="primary"):
        with st.spinner("جارٍ جلب المنتجات..."):
            try:
                st.session_state["salla_token"] = profile["token"]
                data = salla_get_products()
                profile["products"] = data.get("data", [])
            except requests.RequestException as e:
    ...
    if products:
            ...
            for p, issues in need_fix:
                with st.expander(f"⚠️ {p.get('name') or '(بلا اسم)'} — {len(issues)} مشكلة"):
                    for issue in issues:
                        st.write(f"❌ {issue}")
                    st.divider()
                    new_name = st.text_input("الاسم", value=p.get("name", ""), key=f"issue_name_{active}_{p['id']}")
                    current_price = (p.get("price") or {}).get("amount", 0) or 0
                    new_price = st.number_input("السعر", value=float(current_price), key=f"issue_price_{active}_{p['id']}")
                    new_qty = st.number_input("الكمية المتاحة", value=int(p.get("quantity", 0) or 0),
                                               key=f"issue_qty_{active}_{p['id']}")
                    if st.button("💾 حفظ التعديل", key=f"issue_save_{active}_{p['id']}"):
                        try:
        ...
        for p, issues in products_issues:
            ...
            with st.expander(f"{mark}{p.get('name')} — {p.get('price', {}).get('amount', '')} {p.get('price', {}).get('currency', '')}"):
                new_name = st.text_input("الاسم", value=p.get("name", ""), key=f"name_{active}_{p['id']}")
                current_price = p.get("price", {}).get("amount", 0)
                new_price = st.number_input("السعر", value=float(current_price), key=f"price_{active}_{p['id']}")
                new_qty = st.number_input("الكمية المتاحة", value=int(p.get("quantity", 0) or 0),
                                           key=f"qty_{active}_{p['id']}")
                if st.button("💾 حفظ التعديل", key=f"save_{active}_{p['id']}"):
                    try:
                        salla_update_product(p["id"], {
                            "name": new_name, "price": new_price, "quantity": new_qty,
                        })
...
with st.sidebar:
    ...
    ربط مباشر بمتجر سلة لتعديل المنتجات ورصد ما يحتاج مراجعة.<br><br>
    <b style="color:#d4af37;">📁 التقارير</b><br>
    كل التقارير التي أصدرتها في هذه الجلسة، جاهزة لإعادة التحميل.
    </div>
    """, unsafe_allow_html=True)

    with st.expander("🏷️ هوية التقرير (اختياري)"):
        st.text_input("اسم مقدّم الخدمة", key="provider_name",
                       placeholder="مثال: عبدالله البسيوني")
        st.caption("يظهر هذا الاسم أعلى تقارير PDF التي تُصدرها الأداة.")

tab1, tab2, tab3 = st.tabs(["🔍 فحص المتجر", "🛠️ تعديل منتجات سلة", "📁 التقارير"])
with tab1:
    render_audit_tab()
with tab2:
    render_salla_tab()
